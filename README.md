...
*(2026-08-08 업데이트)*
- 다중 파일 업로드 및 파일별 업로드 진행 상태 표시
- 대시보드 NAS 저장공간 사용량 시각화
- NAS 연결/마운트 오류 상황별 안내 메시지 제공
...
*(2026-08-08 업데이트)*
- 여러 개 파일을 한 번에 선택하거나 드래그 앤 드롭으로 업로드
- 업로드 중 전체 진행률, 처리 용량, 파일별 상태 확인
- 업로드 성공 후 파일 목록 자동 새로고침
- 확장자가 없는 일부 영상 파일은 MIME 타입 또는 파일 헤더를 기준으로 확장자 자동 보정
...
- NAS 저장공간 사용률을 원형 차트와 막대 그래프로 표시
- 사용 중 용량, 여유 공간, 전체 용량 확인
- 저장공간 사용률 90% 이상 시 주의 상태 표시
...
## 2026-08-08 업데이트
***파일 업로드 사용성과 NAS 저장공간 확인 기능을 중심으로 개선***

- 파일 관리 화면에서 여러 개 파일을 한 번에 업로드할 수 있도록 다중 파일 업로드 기능 추가
- 드래그 앤 드롭 업로드 영역 추가
- 업로드 진행률, 처리 용량, 파일별 대기/진행/완료/실패 상태 표시 추가
- `/api/files/upload` API 추가
- 기존 `/files/upload`에서도 다중 파일 업로드를 처리하도록 개선
- 관리자 대시보드에 NAS 저장공간 시각화 섹션 추가
- 저장공간 사용률, 사용 중 용량, 여유 공간, 전체 용량 표시
- 저장공간 사용률이 90% 이상일 때 `주의` 상태로 표시
- `/api/storage` API 추가
- 확장자가 없는 영상 파일 업로드/다운로드 시 MIME 타입과 파일 헤더를 기반으로 확장자 추론
- 캠코더/카메라 영상 확장자 분류 추가: `.mts`, `.m2ts`, `.mpg`, `.mpeg`, `.mod`, `.tod`, `.ts`, `.vob`, `.3gp`, `.3g2`
- NAS 경로 접근 실패, 네트워크 미도달, 서버 연결 실패 상황에 대한 오류 메시지 개선
- Raspberry Pi 배포 스크립트에서 NAS 서버/공유명/마운트 경로를 파라미터로 받을 수 있도록 개선
- Tailscale 프록시 SSH 옵션 추가
- SCP 전송 실패 시 legacy SCP 프로토콜로 재시도하도록 배포 안정성 개선